...
import ATM.banco.*;
import ATM.*;
public class Contrasena extends JFrame{  //JFrame que pide contraseña
	...
	public Contrasena(int opcion)
	{
		...
		if(opcion == 1){
			titulo.setText("DIGITE NIP DE ADM");
		}
		else{
			titulo.setText("DIGITE SU NIP");
		}
		titulo.setBounds(320, 100, 270, 30);
		...
		cancelar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				FondoPrincipal nueva_ventana = new FondoPrincipal();
				setVisible(false);
			}
		});
		...
		continuar.addActionListener(new ActionListener(){
			...
			CajeroAutomatico cajeroatm = new CajeroAutomatico(banco);
				if(textPass.equals("0000")||cajeroatm.verificarNIP(textPass)){
					if(opc == 1){
						OpcionesClientes nueva_ventana = new OpcionesClientes();
						setVisible(false);
					}
					else{
						TipoDeCuentas nueva_ventana = new TipoDeCuentas();
						setVisible(false);
					}
				}
				else{
